...
def pulizia_country(df):
    ...
    #coutry_separati = df['country'].str.split(', ')
    return df
...
def grafico_distribuzione(df_pulito):
    ...
    conteggio = df_pulito[['country']].value_counts().head(5)
    
    plt.figure(figsize=(12, 8))
    plt.barh(conteggio.index, conteggio.values, color='skyblue')
...
def raggruppa_e_crea(df):
    new = df['release_year'].value_counts().sort_index()
